...
| `GITHUB_OWNER`, `GITHUB_REPO`, `GITHUB_TOKEN` | Target repo, for the give-up check's closed-PR count and for Minion (inherited — see below) |
| `TARGET_REPO_URL` | Clone URL Minion checks out, e.g. `https://x-access-token:$GITHUB_TOKEN@github.com/owner/repo.git`. Not read by Foreman itself — validated at startup so a missing value fails fast instead of deep inside a Minion run |
...
```json
["docker","run","--rm","-i","-e","TARGET_REPO_URL","-e","GITHUB_OWNER","-e","GITHUB_REPO","-e","GITHUB_TOKEN","minion:latest"]
```
...
docker run --rm -it \
  ...
  -e GITHUB_OWNER -e GITHUB_REPO -e GITHUB_TOKEN -e TARGET_REPO_URL -e MINION_COMMAND \
  foreman:latest
```

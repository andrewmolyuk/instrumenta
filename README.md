...
| `MINION_CLAUDE_MODEL`, `MINION_CLAUDE_EFFORT` *(optional, default `claude-opus-5[1m]` / `high`)* | The model and reasoning effort Minion runs Claude Code with — read directly by Minion (`minion/implement-task.mts`), passed as `--model`/`--effort`. `[1m]` is Opus 5's 1M-context variant. Pinned rather than left to the CLI's own defaults so cost per attempt (ADR-008) doesn't move when those defaults do; override per deployment (add `-e MINION_CLAUDE_MODEL -e MINION_CLAUDE_EFFORT` to `MINION_COMMAND` for it to reach the container) |

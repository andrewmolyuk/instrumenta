...
`docs-consistency-check`'s worked examples and ADR-014's append-only guarantee keep
resolving by number. That's the right call while `docs/decisions/*.md` is read only by

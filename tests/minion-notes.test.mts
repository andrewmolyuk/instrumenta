...
import { attempts, blockedNoVerifyFilename, blockedNoVerifyNote, givenUpFilename, givenUpNote } from '../minion/notes.mts'
...
describe('givenUpNote', () => {
  it('has the docs/todo/ frontmatter convention and mentions the jira_key', () => {
    ...
    expect(note).toMatch(/^---\ntype: bug\nstatus: open\ndate: \d{4}-\d{2}-\d{2}\nsource: minion t1\n---/)
    expect(note).toContain('KAZ-42')
    // Pluralised from MAX_ATTEMPTS, so it reads "1 attempt" rather than "1 attempts".
    expect(note).toContain(attempts(MAX_ATTEMPTS))
    expect(note).not.toMatch(/\b1 attempts\b/)
  })
})
